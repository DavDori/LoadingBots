classdef RectangularCargo < handle
    ...
    methods
        function obj = RectangularCargo(c,c_mass, initial_angle, dim)
            obj.center = c;
        ...
        function r = computeVertexPositions(obj) % calculate the vertex position of the rectangle
            ...
            r(1,1:2) = obj.relative2absolute(v1);
            r(2,1:2) = obj.relative2absolute(v2); 
            r(3,1:2) = obj.relative2absolute(v3); 
            r(4,1:2) = obj.relative2absolute(v4); 
        end
        
        function p2 = relative2absolute(obj, p1)
            if(all(size(p1) == [2,1]) == true)
                p2 = obj.center + rotationMatrix(obj.orientation) * p1;
            else
                error('Need column vector as input!');
            end
        end
